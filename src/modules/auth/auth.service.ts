...
export class AuthService {
  ...
  async signup(signupDto: SignupDto, ipAddress?: string, userAgent?: string) {
    ...
    try {
      ...
      const { emailVerificationRaw } = this.handleEmailVerificationToken(user);
      await userRepo.save(user);
    ...
    try {
      ...
      return {
        ...
        session: {
          ...
          accessTokenExpiresAt: session.access_token_expires_at.toISOString(),
  ...
  async verifyEmail(token: string, ipAddress?: string, userAgent?: string) {
    ...
    try {
      ...
      // ⚠️ Hashing token directly won't work when comparing stored hashed token.
      // Instead, fetch user and use bcrypt.compare() to verify.
    ...
    try {
      ...
      return await this.dataSource.transaction(
        async (manager: EntityManager) => {
          ...
          const isValidRefresh = payload.refreshToken === session.refresh_token;
          if (!isValidRefresh) {
            throw new UnauthorizedException('Invalid refresh token');
          }

          // 🔹 3. Check refresh token expiration
          if (isBefore(session.refresh_token_expires_at, new Date())) {
            throw new UnauthorizedException(
          ...
          if (isBefore(session.refresh_token_expires_at, oneDayFromNow)) {
          ...
          session.ip_address = payload.ipAddress || session.ip_address;
          session.user_agent = payload.userAgent || session.user_agent;
